...
static int rigol_ds_read_header(struct sr_channel *channel)
{
	char start[3], length[10];
	int len, tmp;

	/* Read the hashsign and length digit. */
	tmp = sr_channel_blocking_read(channel, start, 2, 0);
	start[2] = '\0';
	if (tmp != 2)
	{
	...
	tmp = sr_channel_blocking_read(channel, length, len, 0);
	length[len] = '\0';
	if (tmp != len)
	{
...
SR_PRIV int rigol_ds_receive(int fd, int revents, void *cb_data)
{
	...
	if (revents == G_IO_IN) {
		if (devc->model->protocol == PROTOCOL_IEEE488_2) {
			...
			if (devc->num_block_bytes == 0) {
				...
				if (rigol_ds_send(sdi, ":WAV:DATA?") != SR_OK)
					return TRUE;
				len = rigol_ds_read_header(channel);
				if (len == -1)
					return TRUE;
				...
				if (devc->data_source == DATA_SOURCE_LIVE
					...
					sr_channel_blocking_read(channel, devc->buffer, len + 1, 0);
					return TRUE;
				}
		...
		if (devc->model->protocol == PROTOCOL_IEEE488_2) {
			...
			len = sr_channel_blocking_read(channel, devc->buffer,
					len < ACQ_BUFFER_SIZE ? len : ACQ_BUFFER_SIZE, 0);
		} else {
			waveform_size = probe->type == SR_PROBE_ANALOG ?
					DS1000_ANALOG_LIVE_WAVEFORM_SIZE : DIGITAL_WAVEFORM_SIZE;
			len = sr_channel_blocking_read(channel, devc->buffer,
					waveform_size - devc->num_frame_bytes, 0);
		}
			...
			if (devc->model->protocol == PROTOCOL_IEEE488_2) {
				if (devc->num_block_read == devc->num_block_bytes) {
					...
					/* Discard the terminating linefeed and prepare for
					   possible next block */
					sr_channel_blocking_read(channel, devc->buffer, 1, 0);
					devc->num_block_bytes = 0;
					if (devc->data_source != DATA_SOURCE_LIVE)
						rigol_ds_set_wait_event(devc, WAIT_BLOCK);
				} else
...
SR_PRIV int rigol_ds_send(const struct sr_dev_inst *sdi, const char *format, ...)
{
	...
	char buf[256];
	int len, out, ret;
	struct sr_channel *channel = sdi->conn;
	...
	strcat(buf, "\n");
	len++;
	out = sr_channel_blocking_write(channel, buf, len, 0);
	buf[len - 1] = '\0';
	if (out != len) {
...
static int get_cfg(const struct sr_dev_inst *sdi, char *cmd, char *reply, size_t maxlen)
{
	...
	if ((len = sr_channel_blocking_read(channel, reply, maxlen - 1, 0)) < 0)
		return SR_ERR;
	reply[len] = '\0';
